...
public sealed class TaggedEnumSourceGenerator: IIncrementalGenerator {
	...
	private static readonly DiagnosticDescriptor Rule3 = new("TAGGED0004",
		"Invalid type argument",
		"The type arguments of [Data<T>] must be consistent with [Tagged<T>]",
		"SourceGenerator", DiagnosticSeverity.Error, true,
		"The type arguments of [Data<T>] must be consistent with [Tagged<T>], default is string.");
	...
	public void Initialize(IncrementalGeneratorInitializationContext context) {
		...
		foreach (var taggedAttrName in taggedAttrs) {
			...
			var dataDiagnosticProvider = dataProvider.Where(static v => v.Members.Any(static m => m.Diagnostics is not null)).SelectMany(static (v, _) => v.Members!).Select(static (v, _) => v.Diagnostics).Collect();
			...
			var diagnosticProvider = dataDiagnosticProvider.Combine(taggedDiagnosticProvider)
				.Select(static (v, _) => (v.Left.Aggregate(Enumerable.Empty<Diagnostic>(), static (cur, next) => cur.Concat(next ?? [])) ?? []).Concat(v.Right.Aggregate(Enumerable.Empty<Diagnostic>(), static (cur, next) => cur.Concat(next ?? []))));
	...
	private void GenerateSource(SourceProductionContext ctx, TargetEnumInfo data) {
		...
		var nameValueMap = data.Members.Select(m => $@"{{{data.TypeName}.{m.MemberName}, ""{data.TypeName}.{m.MemberName}""}},").Aggregate("", static (current, next) => current + "\n\t\t" + next);
		var valueNameMap = data.Members.Select(m => $@"{{""{m.MemberName}"", {data.TypeName}.{m.MemberName}}},").Aggregate("", static (current, next) => current + "\n\t\t" + next);
		var valueDataConditionalBranches = data.Members.Where(static m => m.HasData).Select(m => $"{data.TypeName}.{m.MemberName} => ({m.TypeName}){m.Data},").Aggregate("", static (current, next) => current + "\n\t\t\t" + next);
		...
		var valueNameConditionalBranches = data.Members.Select(m => $@"{data.TypeName}.{m.MemberName} => ""{m.MemberName}"",").Aggregate("", static (current, next) => current + "\n\t\t\t" + next);
		var source = $$"""
		// <auto-generated />
		#nullable enable
