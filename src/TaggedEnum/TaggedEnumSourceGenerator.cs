...
public sealed class TaggedEnumSourceGenerator: IIncrementalGenerator {
				...
				public {{data.DataTypeName}} Data {
					...
					get =>
						...
						{{valueDataConditionalBranches}}
							_ => DataNotFoundException.ThrowWithMessage<{{data.DataTypeName}}>($"Data of {ValueNameMap[self]} not found.")
						};
				}
		""" : $$"""
			public {{data.DataTypeName}} Data {
				...
				get {
					if (ValueDataMap.TryGetValue(self, out var data)) {
						...
						return DataNotFoundException.ThrowWithMessage<{{data.DataTypeName}}>($"Data of {ValueNameMap[self]} not found.");
					}
				}
			}
		""";
		...
		public sealed class {{formattedTypeName}}ToDataConverter: JsonConverter<{{data.TypeName}}> {
			public override {{data.TypeName}} Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
				...
				return {{(data.DataTypeName == "string" ? $$"""
					...
					? JsonException.ThrowWithMessage<{{data.TypeName}}>($"Couldn't convert data \"{token}\" to {{data.TypeName}}.")
					: {{formattedTypeName}}Extension.TryGetValueByData(token, out var result)
						? result.Value : JsonException.ThrowWithMessage<{{data.TypeName}}>($"Couldn't find \"{token}\" in {{data.TypeName}} data.")
				""" : $$"""
					{{formattedTypeName}}Extension.TryGetValueByData(token, out var result)
						? result.Value : JsonException.ThrowWithMessage<{{data.TypeName}}>($"Couldn't find \"{token}\" in {{data.TypeName}} data.")
				""")}};
					...
					switch (reader.TokenType) {
						...
						case JsonTokenType.String:
							...
							list.Add({{formattedTypeName}}Extension.TryGetValueByData(
								v,
								out var item)
								? item!.Value : JsonException.ThrowWithMessage<{{data.TypeName}}>($"Couldn't find \"{v}\" in {{data.TypeName}} data."));
							continue;
						default:
							continue;
					}
				...
				while (reader.Read()) {
					switch (reader.TokenType) {
						...
						case JsonTokenType.EndArray:
							goto label;
						case JsonTokenType.String:
							var v = {{ReaderGetToken("string")}};
							list.Add({{formattedTypeName}}Extension.TryGetValueByName(
								v,
								out var item)
								? item!.Value : JsonException.ThrowWithMessage<{{data.TypeName}}>($"Couldn't find \"{v}\" in {{data.TypeName}} name."));
							continue;
						default:
							continue;
					}
	...
	private static string PrimitiveTypeToReadJsonType(string type)
		=> type switch {
			...
			"double" => "GetDouble()",
			"decimal" => "GetDecimal()",
			"char" => "GetString()?[0] ?? JsonException.ThrowWithMessage<char>(\"Couldn't convert empty string to char\")",
			"string" => "GetString()",
			"bool" => "GetBoolean()",
			_ => ArgumentException.ThrowWithMessage<string>("Invalid type name.")
		};
	...
	public static string ReaderGetToken(string type)
		=> type == "string"
				? $@"reader.GetString() is not {{}} token ? UnreachableException.ThrowWithMessage<string>(""JsonTokenType is string but reader.GetString() is null."") : token"
				: ZString.Concat("reader.", PrimitiveTypeToReadJsonType(type));

	// [MethodImpl(MethodImplOptions.AggressiveInlining)]
	// public static string TrimStartLine(int n, string str) {
