...
public sealed class TaggedEnumSourceGenerator: IIncrementalGenerator {
	...
	private void GenerateSource(SourceProductionContext ctx, FinalResult data) {
		...
		var nameValueMap = data.Members.Select(m => $@"{{{data.Type.TypeName}.{m.MemberName}, ""{m.MemberName}""}},").Aggregate("", static (current, next) => current + "\n\t\t" + next);
		var conditionalBranches = data.Members.Where(static m => m.HasValue).Select(m => $"{data.Type.TypeName}.{m.MemberName} => {m.Value},").Aggregate("", static (current, next) => current + "\n\t\t\t" + next);
		var source = $$"""
		// <auto-generated />
		#nullable enable
		using System.Runtime.CompilerServices;
	...
	private static FinalResult ProcessMembers(TypeInfo v, CancellationToken cancellationToken) {
		// if (!Debugger.IsAttached) {
		// 	Debugger.Launch();
		// }
		var valueType = v.SemanticModel.Compilation.GetTypeByMetadataName("TaggedEnum.Value");
		var valueGenericType = v.SemanticModel.Compilation.GetTypeByMetadataName("TaggedEnum.Value`1");
		var memberValueMap = v.Symbol.GetMembers()
			.OfType<IFieldSymbol>()
			.Select((m, cancellationToken) => {
				...
				if (valueAttrCount > 1) {
					...
					if (valueAttrData.ConstructorArguments.Length > 0) {
						...
						if (value.Type?.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) != v.ValueTypeName) {
							// TODO error
						} else {
							memberInfo.Value = value.Value is string str ? $@"""{str}""" : value.Value?.ToString() ?? string.Empty;
							memberInfo.HasValue = true;
						}
					} else if (v.ValueTypeName != TypeInfo.DefaultType) {
						// TODO error
					} else {
						memberInfo.Value = $@"""{m.Name}""";
						memberInfo.HasValue = true;
